...
#define FTM_PWM_IP_TYPES_SW_MAJOR_VERSION                      3
...
typedef enum
{
    ...
    FTM_PWM_IP_SYNC_ON_CLK     = 0x01U,    /*!< synchronization for the FTM register is done on each FTM Clock rising edge */
    FTM_PWM_IP_SYNC_ON_TRIGGER = 0x02U     /*!< synchronization for the FTM register is done on each FTM Sync Trigger */
} Ftm_Pwm_Ip_SyncType;
...
typedef struct
{
    ...
    boolean ObeCtrl;                                  /*!< Selected the FTM channel output state */
#endif
