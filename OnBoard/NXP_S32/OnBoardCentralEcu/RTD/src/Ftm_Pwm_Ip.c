...
#if ((STD_ON == FTM_PWM_IP_USER_ACCESS_ALLOWED_AVAILABLE) || ((STD_ON == FTM_PWM_IP_SIM_AVAILABLE) && ((STD_ON == FTM_PWM_IP_ENABLE_USER_MODE_SUPPORT))))
...
#define FTM_PWM_IP_SW_MAJOR_VERSION_C                  3
...
==================================================================================================*/
    ...
    #if ((FTM_PWM_IP_AR_RELEASE_MAJOR_VERSION_C != DEVASSERT_AR_RELEASE_MAJOR_VERSION) || \
         (FTM_PWM_IP_AR_RELEASE_MINOR_VERSION_C != DEVASSERT_AR_RELEASE_MINOR_VERSION))
        #error "AUTOSAR version numbers of Ftm_Pwm_Ip.c and Devassert.h are different."
    #endif
...
*                                       GLOBAL VARIABLES
===============================================================================================*/
#if (FTM_PWM_IP_NO_CACHE_NEEDED == STD_ON)
...
#endif
#if (FTM_PWM_IP_NOTIFICATION_SUPPORTED == STD_OFF)
...
#if (FTM_PWM_IP_NO_CACHE_NEEDED == STD_ON)
    #define PWM_START_SEC_VAR_CLEARED_BOOLEAN_NO_CACHEABLE
#else
    #define PWM_START_SEC_VAR_CLEARED_BOOLEAN
#endif
#include "Pwm_MemMap.h"

/*! @brief Arrays to indicate a channel using the software output */
static boolean Ftm_Pwm_Ip_aChannelSoftOutputUsed[FTM_PWM_IP_INSTANCE_COUNT][FTM_PWM_IP_CHANNEL_COUNT];

/*! @brief Arrays to indicate a channel using the software output at the initialization step */
static boolean Ftm_Pwm_Ip_aChannelSoftOutputUsedAtInit[FTM_PWM_IP_INSTANCE_COUNT][FTM_PWM_IP_CHANNEL_COUNT];

#if (FTM_PWM_IP_NO_CACHE_NEEDED == STD_ON)
    #define PWM_STOP_SEC_VAR_CLEARED_BOOLEAN_NO_CACHEABLE
#else
    #define PWM_STOP_SEC_VAR_CLEARED_BOOLEAN
#endif
#include "Pwm_MemMap.h"
...
void Ftm_Pwm_Ip_SetSimOutputBufferEn(Ftm_Pwm_Ip_SimType * const SimBase,
                                     uint8                      Instance,
                                     boolean                    ObeCtrlEn);
#endif
...
static void Ftm_Pwm_Ip_SetChnTriggerAndSoftwareCtrl(uint8 Instance, const Ftm_Pwm_Ip_ChannelConfigType * UserChCfg, boolean SwControlEn, boolean SwControlVal);

static void Ftm_Pwm_Ip_CalSwCtrlEnAndSwCtrlValCh(uint8 Instance,
...
#endif
static void Ftm_Pwm_Ip_ResetSoftCtrlOfAllChsNotConf(uint8 Instance, const Ftm_Pwm_Ip_UserCfgType * UserCfg);
                                               ...
                                               uint8 Channel)
{
    ...
    if ((TRUE == Ftm_Pwm_Ip_IsEnableFaultControlMode(FtmBase)) && (TRUE == Ftm_Pwm_Ip_IsEnableFaultInput(FtmBase)))
    {
       ...
       /* Clear time-overflow interrupt flag of a given FTM module */
                                                 ...
                                                 boolean ActiveState)
{
    ...
    else
    {
        ...
        {
            /* Clear CHnF bit interrupt flag of a given channel */
...
static void Ftm_Pwm_Ip_ConfigureSWandHWSync(uint8 Instance, const Ftm_Pwm_Ip_SyncCfgType * SyncCfg)
{
    ...
    switch (SyncCfg->SyncMode)
    {
        case FTM_PWM_IP_SYNC_SWTRIGGER:
            ...
            Ftm_Pwm_Ip_SetOutmaskSoftwareSyncModeCmd(FtmBase, (SyncCfg->OutMaskSync == FTM_PWM_IP_SYNC_ON_TRIGGER));
            Ftm_Pwm_Ip_SetOutmaskHardwareSyncModeCmd(FtmBase, FALSE);
            /* Configure sync for INVCTRL register */
            Ftm_Pwm_Ip_SetInvctrlSoftwareSyncModeCmd(FtmBase, (SyncCfg->InverterSync == FTM_PWM_IP_SYNC_ON_TRIGGER));
            Ftm_Pwm_Ip_SetInvctrlHardwareSyncModeCmd(FtmBase, FALSE);
            /* Configure sync for SWOCTRL register */
            Ftm_Pwm_Ip_SetSwoctrlSoftwareSyncModeCmd(FtmBase, (SyncCfg->OutRegSync == FTM_PWM_IP_SYNC_ON_TRIGGER));
            Ftm_Pwm_Ip_SetSwoctrlHardwareSyncModeCmd(FtmBase, FALSE);
            ...
            Ftm_Pwm_Ip_SetCounterSoftwareSyncModeCmd(FtmBase, (SyncCfg->CounterSync == FTM_PWM_IP_SYNC_ON_TRIGGER));
        ...
        case FTM_PWM_IP_SYNC_HWTRIGGER:
            ...
            Ftm_Pwm_Ip_SetOutmaskHardwareSyncModeCmd(FtmBase, (SyncCfg->OutMaskSync == FTM_PWM_IP_SYNC_ON_TRIGGER));
            /* Configure sync for INVCTRL register */
            Ftm_Pwm_Ip_SetInvctrlSoftwareSyncModeCmd(FtmBase, FALSE);
            Ftm_Pwm_Ip_SetInvctrlHardwareSyncModeCmd(FtmBase, (SyncCfg->InverterSync == FTM_PWM_IP_SYNC_ON_TRIGGER));
            /* Configure sync for SWOCTRL register */
            Ftm_Pwm_Ip_SetSwoctrlSoftwareSyncModeCmd(FtmBase, FALSE);
            Ftm_Pwm_Ip_SetSwoctrlHardwareSyncModeCmd(FtmBase, (SyncCfg->OutRegSync == FTM_PWM_IP_SYNC_ON_TRIGGER));
            /* Configure sync for MOD, HCR, CNTIN, and CV registers */
            ...
            Ftm_Pwm_Ip_SetCounterHardwareSyncModeCmd(FtmBase, (SyncCfg->CounterSync == FTM_PWM_IP_SYNC_ON_TRIGGER));
            break;
...
static void Ftm_Pwm_Ip_ConfigureSyncType(uint8 Instance, const Ftm_Pwm_Ip_SyncCfgType * SyncCfg)
{
    ...
    /* Configure sync for OUTMASK register */
    Ftm_Pwm_Ip_SetOutmaskPwmSyncModeCmd(FtmBase, (SyncCfg->OutMaskSync == FTM_PWM_IP_SYNC_ON_CLK) ? FALSE : TRUE);
    /* Configure sync for INVCTRL register */
    Ftm_Pwm_Ip_SetInvctrlPwmSyncModeCmd(FtmBase, (SyncCfg->InverterSync == FTM_PWM_IP_SYNC_ON_CLK) ? FALSE : TRUE);
    /* Configure sync for SWOCTRL register */
    Ftm_Pwm_Ip_SetSwoctrlPwmSyncModeCmd(FtmBase, (SyncCfg->OutRegSync == FTM_PWM_IP_SYNC_ON_CLK) ? FALSE : TRUE);
    /* Configure sync for CNTIN register */
    Ftm_Pwm_Ip_SetCntinPwmSyncModeCmd(FtmBase, (SyncCfg->InitCounterSync == FTM_PWM_IP_SYNC_ON_CLK) ? FALSE : TRUE);
}
...
static void Ftm_Pwm_Ip_NormalCheckNotification(uint8 Instance, uint8 Channel)
{
         ...
         ( FTM_PWM_IP_MODE_COMBINE_LOW           == ChMode))
    {
        ...
        {
            /* Clear CHnF bit interrupt flag of a given channel */
            Ftm_Pwm_Ip_ClearChnEventFlag(FtmBase, (uint8)(PairChId * 2U));
        ...
        {
            /* Clear CHnF bit interrupt flag of a given channel */
            Ftm_Pwm_Ip_ClearChnEventFlag(FtmBase, (uint8)((PairChId * 2U) + 1U));
...
void Ftm_Pwm_Ip_SetSimOutputBufferEn(Ftm_Pwm_Ip_SimType * const SimBase,
                                     ...
                                     boolean                    ObeCtrlEn)
{
    uint32 ObeControlEn = ((ObeCtrlEn == TRUE)? (uint32)1U : (uint32)0U);
    SimBase->MISCTRL0   = (SimBase->MISCTRL0  & ~(uint32)((uint32)SIM_MISCTRL0_FTM0_OBE_CTRL_MASK << Instance)) | (((uint32)ObeControlEn << Instance) << SIM_MISCTRL0_FTM0_OBE_CTRL_SHIFT);
}
#endif
...
{
    Ftm_Pwm_Ip_HwAddrType * FtmBase = Ftm_Pwm_Ip_aFtmBase[Instance];
    const Ftm_Pwm_Ip_InstanceCfgType * UserInstCfg = UserCfg->InstanceCfg;
...
#if (defined(FTM_PWM_IP_SIM_AVAILABLE) && (FTM_PWM_IP_SIM_AVAILABLE == STD_ON))
    SchM_Enter_Pwm_PWM_EXCLUSIVE_AREA_66();
#if (defined(MCAL_ENABLE_USER_MODE_SUPPORT) && (STD_ON == FTM_PWM_IP_ENABLE_USER_MODE_SUPPORT))
    /* The FTM channel output state is retained when the channel is in output mode.*/
    OsIf_Trusted_Call3params(Ftm_Pwm_Ip_SetSimOutputBufferEn, SimBase, Instance, UserInstCfg->ObeCtrl);
#else
    /* The FTM channel output state is retained when the channel is in output mode.*/
    Ftm_Pwm_Ip_SetSimOutputBufferEn(SimBase, Instance, UserInstCfg->ObeCtrl);
#endif
    SchM_Exit_Pwm_PWM_EXCLUSIVE_AREA_66();
#endif

    Ftm_Pwm_Ip_ResetAndFirstConfigure(Instance, UserInstCfg);

    /* Reset all CHnOC bits and CHnOCV bits in SWOCTRL register of channels not configured */
    Ftm_Pwm_Ip_ResetSoftCtrlOfAllChsNotConf(Instance, UserCfg);
...
static void Ftm_Pwm_Ip_SetChnTriggerAndSoftwareCtrl(uint8 Instance, const Ftm_Pwm_Ip_ChannelConfigType * UserChCfg, boolean SwControlEn, boolean SwControlVal)
{
    ...
    if (TRUE == SwControlEn)
    {
        ...
        Ftm_Pwm_Ip_SetChnSoftwareCtrlCmd(FtmBase, UserChCfg->ChannelId, SwControlEn);

        /* Configure Channel Software Output Control Value */
        Ftm_Pwm_Ip_SetChnSoftwareCtrlVal(FtmBase, UserChCfg->ChannelId, SwControlVal);
    }
    else
    {
        /* Configure Channel Software Output Control Value */
    ...
    if (TRUE == Ftm_Pwm_Ip_aChannelSoftOutputUsedAtInit[Instance][UserChCfg->ChannelId])
    {
        if (NULL_PTR != UserChCfg->PairCfg)
        {
            if (TRUE == UserChCfg->PairCfg->ComplementaryModeEn)
            {
                /* Configure  Software Output Control Enable for channel (n+1) */
                Ftm_Pwm_Ip_SetChnSoftwareCtrlCmd(FtmBase, UserChCfg->ChannelId + (uint8)1U, SwControlEn);

                if (FTM_PWM_IP_MAIN_DUPLICATED == UserChCfg->PairCfg->PairChPolarity)
                {
                    /* Configure Channel Software Output Control Value for channel (n+1) */
                    Ftm_Pwm_Ip_SetChnSoftwareCtrlVal(FtmBase, UserChCfg->ChannelId + (uint8)1U, SwControlVal);
                }
                else
                {
                    /* Configure Channel Software Output Control Value for channel (n+1) */
                    Ftm_Pwm_Ip_SetChnSoftwareCtrlVal(FtmBase, UserChCfg->ChannelId + (uint8)1U, (SwControlVal == TRUE) ? FALSE : TRUE);
                }
            }
        }
    }
...
/*FUNCTION**********************************************************************
 *
 * Function Name : Ftm_Pwm_Ip_CalSwCtrlEnAndSwCtrlValCh
 * Description   : Calculate the value of CHnOCV bit and CHnOC bit for the corresponding channel in case the duty cycle value configured: 0% or 100%.
 *
 *END**************************************************************************/
static void Ftm_Pwm_Ip_CalSwCtrlEnAndSwCtrlValCh(uint8 Instance,
                                                 const Ftm_Pwm_Ip_ChannelConfigType * UserChCfg,
                                                 ...
                                                 boolean * SwControlVal)
{
    if (((UserChCfg->DutyCycle == Ftm_Pwm_Ip_aPeriod[Instance]) && (FTM_PWM_IP_POLARITY_HIGH == UserChCfg->Polarity)) ||
        ((0U                   == UserChCfg->DutyCycle)         && (FTM_PWM_IP_POLARITY_LOW  == UserChCfg->Polarity)))
    {
        /* Force the output signal of channel to one */
        *SwControlEn  = TRUE;
        *SwControlVal = TRUE;

        Ftm_Pwm_Ip_aChannelSoftOutputUsedAtInit[Instance][UserChCfg->ChannelId] = TRUE;
        Ftm_Pwm_Ip_aChannelSoftOutputUsed[Instance][UserChCfg->ChannelId]       = TRUE;
    }
    else if (((UserChCfg->DutyCycle == Ftm_Pwm_Ip_aPeriod[Instance]) && (FTM_PWM_IP_POLARITY_LOW   == UserChCfg->Polarity)) ||
             ((0U                   == UserChCfg->DutyCycle)         && (FTM_PWM_IP_POLARITY_HIGH  == UserChCfg->Polarity)))
    {
        /* Force the output signal of channel to zero */
        *SwControlEn  = TRUE;
        *SwControlVal = FALSE;
                                   ...
                                   const Ftm_Pwm_Ip_ChannelConfigType * UserChCfg)
{
    ...
    /* Configure the PWM channel */
    ChannelId = UserChCfg->ChannelId;
    /* Configure the software output control of channel */
    boolean SwControlEn  = UserChCfg->SwControlEn;
    boolean SwControlVal = UserChCfg->SwControlVal;
    Ftm_Pwm_Ip_aChannelSoftOutputUsedAtInit[Instance][UserChCfg->ChannelId] = FALSE;
    Ftm_Pwm_Ip_aChannelSoftOutputUsed[Instance][UserChCfg->ChannelId]       = FALSE;
    ...
    /* The software output control feature's used to avoid a spike pulse when using multiple variants in case the value of duty cycle: 0% or 100% */
    if ((UserChCfg->DutyCycle == Ftm_Pwm_Ip_aPeriod[Instance]) || (0U == UserChCfg->DutyCycle))
    {
        Ftm_Pwm_Ip_CalSwCtrlEnAndSwCtrlValCh(Instance, UserChCfg, &SwControlEn, &SwControlVal);
    }
    else
    {
        /* Do nothing */
    ...
    Ftm_Pwm_Ip_SetChnTriggerAndSoftwareCtrl(Instance, UserChCfg, SwControlEn, SwControlVal);
...
/*FUNCTION**********************************************************************
 *
 * Function Name : Ftm_Pwm_Ip_ResetSoftCtrlOfAllChsNotConf
 * Description   : Reset all CHnOC bits and CHnOCV bits in SWOCTRL register of channels not configured .
 *
 *END**************************************************************************/
static void Ftm_Pwm_Ip_ResetSoftCtrlOfAllChsNotConf(uint8 Instance, const Ftm_Pwm_Ip_UserCfgType * UserCfg)
{
    ...
    uint8 Index                     = 0U;
    uint8 Index1                    = 0U;
    boolean IsChannelConfigured     = FALSE;
    ...
    for (Index = 0U; Index < FTM_PWM_IP_CHANNEL_COUNT; Index++)
    {
        ...
        for (Index1 = 0U; Index1 < UserCfg->NoOfConfiguredCh; Index1++)
        {
            if (Index == UserCfg->ConfiguredChArray[Index1]->ChannelId)
            {
        ...
        if (FALSE == IsChannelConfigured)
        {
            /* Reset CHnOC bit */
            Ftm_Pwm_Ip_SetChnSoftwareCtrlCmd(FtmBase, Index, FALSE);
            /* Reset CHnOCV bit */
            Ftm_Pwm_Ip_SetChnSoftwareCtrlVal(FtmBase, Index, FALSE);
        }
    }
}
                     ...
                     const Ftm_Pwm_Ip_UserCfgType * UserCfg)
{
    ...
    Ftm_Pwm_Ip_InitInstance(Instance, UserCfg);

    /* Configure all channels in the user cfg */
    for (Index = 0U; Index < UserCfg->NoOfConfiguredCh; Index++)
    {
    ...
    {
        for (Index = 0U; Index < UserCfg->NoOfFaultCfg; Index++)
        {
 ...
 *
 * @implements     Ftm_Pwm_Ip_UpdatePwmDutyCycleChannel_Activity
 *END**************************************************************************/
                                          ...
                                          boolean SoftwareTrigger)
{
    ...
    if(((Ftm_Pwm_Ip_ChannelStateType)FTM_PWM_IP_CHANNEL_IDLE == Ftm_Pwm_Ip_aChannelState[Instance][Channel])                ||
       ((TRUE                                                == Ftm_Pwm_Ip_aChannelSoftOutputUsed[Instance][Channel])       &&
        (FALSE                                               == Ftm_Pwm_Ip_aChannelSoftOutputUsedAtInit[Instance][Channel])))
    {
        ...
        Ftm_Pwm_Ip_aChannelSoftOutputUsed[Instance][Channel] =  FALSE;
                                ...
                                boolean ActiveState)
{
    ...
    uint16 SwoCtrlRegValTemp        = 0U;
    uint16 ChannelMask              = 0U;
    uint16 SwoCtrlBitMask           = 0U;
    uint16 TempPolVal               = 0U;
    /* This variable used to store the value of CHnOCV bit for the corresponding channel before calculating */
    uint16  ValChOcvBitPre          = 0U;
    /* This variable used to store the value of CHnOCV bit for the corresponding channel after calculating */
    uint16  ValChOcvBitAfterCal     = 0U;
    ...
    SwoCtrlRegValTemp = SwoCtrlRegVal;
    ...
    if (TRUE == ActiveState)
    {
        ...
        ValChOcvBitPre      = (uint16)((SwoCtrlRegValTemp & (uint16)((uint16)1U << ((uint16)Channel + FTM_SWOCTRL_CH0OCV_SHIFT))) >> ((uint16)Channel + FTM_SWOCTRL_CH0OCV_SHIFT));
        ValChOcvBitAfterCal = (uint16)((SwoCtrlRegVal & (uint16)((uint16)1U << ((uint16)Channel + FTM_SWOCTRL_CH0OCV_SHIFT))) >> ((uint16)Channel + FTM_SWOCTRL_CH0OCV_SHIFT));

        if (Ftm_Pwm_Ip_aChannelState[Instance][Channel] != FTM_PWM_IP_CHANNEL_UNINIT)
        {
            /* If the value of original CHnOCV bit and its after caculating are the same, invert polarity to trigger a update for this bit */
            if (ValChOcvBitPre == ValChOcvBitAfterCal)
            {
                if (Ftm_Pwm_Ip_GetChnOutputPolarity(FtmBase, Channel) == FTM_PWM_IP_POLARITY_HIGH)
                {
                    Ftm_Pwm_Ip_SetChnOutputPolarityCmd(FtmBase, Channel, FTM_PWM_IP_POLARITY_LOW);
                }
                else
                {
                    Ftm_Pwm_Ip_SetChnOutputPolarityCmd(FtmBase, Channel, FTM_PWM_IP_POLARITY_HIGH);
                }

                SwoCtrlRegVal &= (uint16)(~((uint16)1U << ((uint16)Channel + FTM_SWOCTRL_CH0OCV_SHIFT)));
                SwoCtrlRegVal |= (uint16)(((uint16)(~ValChOcvBitAfterCal) << ((uint16)Channel + FTM_SWOCTRL_CH0OCV_SHIFT)) & (uint16)((uint16)1U << ((uint16)Channel + FTM_SWOCTRL_CH0OCV_SHIFT)));
            }
        }
    ...
    /* Clear bit SWSYNC before writing 1 to this bit to make sure a new PWM synchronization started */
    Ftm_Pwm_Ip_SetSoftwareTriggerCmd(FtmBase, FALSE);
    ...
    if (Ftm_Pwm_Ip_aChannelState[Instance][Channel] != FTM_PWM_IP_CHANNEL_UNINIT)
    {
        if (ValChOcvBitPre == ValChOcvBitAfterCal)
        {
            Ftm_Pwm_Ip_SetChnOutputPolarityCmd(FtmBase, Channel, ((((TempPolVal >> FTM_SWOCTRL_CH0OCV_SHIFT) & (1U << (uint16)Channel)) >> (uint16)Channel) == 0U) ? FTM_PWM_IP_POLARITY_HIGH : FTM_PWM_IP_POLARITY_LOW);
        }
    }
}
                                                  ...
                                                  boolean SoftwareTrigger)
{
    ...
    if ((TRUE  == Ftm_Pwm_Ip_aChannelSoftOutputUsed[Instance][Channel])       &&
        (FALSE == Ftm_Pwm_Ip_aChannelSoftOutputUsedAtInit[Instance][Channel]))
    {
        Ftm_Pwm_Ip_aChannelSoftOutputUsed[Instance][Channel] =  FALSE;
        /* Set output for current channel*/
                                   ...
                                   Ftm_Pwm_Ip_NotifType NotifType)
{
    ...
    if((FTM_PWM_IP_CHANNEL_0_NOTIFICATION == NotifType) ||
       ...
       (FTM_PWM_IP_CHANNEL_7_NOTIFICATION == NotifType))
    {
        /* Clear match-compare interrupt flag of a given channel */
                                  ...
                                  boolean SoftwareTrigger)
{
    ...
    {
        if ((TRUE  == Ftm_Pwm_Ip_aChannelSoftOutputUsed[Instance][Channels[Index]])       &&
            (FALSE == Ftm_Pwm_Ip_aChannelSoftOutputUsedAtInit[Instance][Channels[Index]]))
        {
            Ftm_Pwm_Ip_aChannelSoftOutputUsed[Instance][Channels[Index]] =  FALSE;
            /* Set output for current channel*/
            Ftm_Pwm_Ip_SwOutputControl(Instance,
                                       Channels[Index],
                                       FTM_PWM_IP_OUTPUT_STATE_LOW,
