...
#define PDB_ADC_IP_SW_MAJOR_VERSION_C               3
                     ...
                     const Pdb_Adc_Ip_ConfigType * const Config)
{
    ...
    {
        /* Clear all sequence error flags triggered from the current channel by writing 0 */
        Base->CH[ChannelIndex].S &= ~(PDB_S_ERR_MASK);
    }
    ...
    Base->IDLY = PDB_IDLY_IDLY(Config->InterruptDelayPeriod);
    /* Interrupt delay notification */
    Pdb_Adc_Ip_axState[Instance].IntDelayNotification = Config->IntDelayNotification;
...
/*FUNCTION*********************************************************************
 *
 * Function Name : Pdb_Adc_Ip_InterruptDelayEnable
 * Description   : Enables Pdb delay interrupt.
 *
 * @implements     Pdb_Adc_Ip_InterruptDelayEnable_Activity
 *END*************************************************************************/
void Pdb_Adc_Ip_InterruptDelayEnable(const uint32 Instance)
{
#if (PDB_ADC_IP_DEV_ERROR_DETECT == STD_ON)
    DevAssert(Instance < PDB_INSTANCE_COUNT);
#endif /* (PDB_ADC_IP_DEV_ERROR_DETECT == STD_ON) */
    PDB_Type * const Base = Pdb_Adc_Ip_apxBase[Instance];
    /* Read SC register value */
    uint32 RegVal = Base -> SC;
    /* Clear delay interrupt flag */
    RegVal &= ~(PDB_SC_PDBIF_MASK);
    /* Enable delay interrupt */
    RegVal |= PDB_SC_PDBIE_MASK;
...
/*FUNCTION*********************************************************************
 *
 * Function Name : Pdb_Adc_Ip_InterruptDelayDisable
 * Description   : Disables Pdb delay interrupt.
 *
 * @implements     Pdb_Adc_Ip_InterruptDelayDisable_Activity
 *END*************************************************************************/
void Pdb_Adc_Ip_InterruptDelayDisable(const uint32 Instance)
{
#if (PDB_ADC_IP_DEV_ERROR_DETECT == STD_ON)
    DevAssert(Instance < PDB_INSTANCE_COUNT);
#endif /* (PDB_ADC_IP_DEV_ERROR_DETECT == STD_ON) */
...
/*FUNCTION*********************************************************************
 *
 * Function Name : Pdb_Adc_Ip_GetInterruptDelayState
 * Description   : Checking if interrupt delay was enabled
 *
 *END*************************************************************************/
boolean Pdb_Adc_Ip_GetInterruptDelayState(const uint32 Instance)
{
#if (PDB_ADC_IP_DEV_ERROR_DETECT == STD_ON)
    DevAssert(Instance < PDB_INSTANCE_COUNT);
#endif /* (PDB_ADC_IP_DEV_ERROR_DETECT == STD_ON) */
    PDB_Type * const Base = Pdb_Adc_Ip_apxBase[Instance];
    uint32 RegValue = (uint32)Base->SC;

    RegValue = (RegValue & PDB_SC_PDBIE_MASK) >> PDB_SC_PDBIE_SHIFT;
...
#if (PDB_ADC_IP_ENABLE_INTERRUPT_DELAY == STD_ON)
        /* CPR_RTD_00664
         * Check if it is spurious interupt */
        if (((Base->SC & PDB_SC_PDBIE_MASK) != 0u) && ((Base->SC & PDB_SC_PDBIF_MASK) != 0u))
        {
            Base->SC &= ~(PDB_SC_PDBIF_MASK);
            if(Pdb_Adc_Ip_axState[Instance].IntDelayNotification != NULL_PTR)
            {
                Pdb_Adc_Ip_axState[Instance].IntDelayNotification(Instance);
            }
        }
#endif /* (PDB_ADC_IP_ENABLE_INTERRUPT_DELAY == STD_ON) */
    }
