...
*
*/
void Pwm_Ipw_SetDutyCycle(uint16                           DutyCycle,
...
#if (PWM_FTM_USED == STD_ON)
    uint8 ChannelIdx                                 = 0U;
    /* The index of painred channel */
    uint8 ChannelPairIdx                             = 0U;
    /* The type of edge interrupt according to the corresponding channel */
    Ftm_Pwm_Ip_EdgeInterruptType TypeOfEdgeInterrupt = FTM_PWM_IP_TOF_IRQ_EDGE;
#endif
    ...
    switch(IpConfig->ChannelType)
    {
        ...
        case PWM_CHANNEL_FTM:
            if (NULL_PTR != IpConfig->FtmChConfig)
            {
                ...
                /* Get the channel ID */
                ChannelIdx     = ((const Ftm_Pwm_Ip_ChannelConfigType *)IpConfig->FtmChConfig)->ChannelId;
                ChannelPairIdx = ChannelIdx;
                /* Get the notification type corresponding channel ID */
                ...
                {
                    ChannelPairIdx      = ChannelIdx + 1U;
                    TypeOfEdgeInterrupt = FTM_PWM_IP_CHF_IRQ_EDGE;
                }
                else
                {
                    /* Do nothing */
                ...
                switch (Notification)
                {
                    case PWM_BOTH_EDGES:
                    {
                        Ftm_Pwm_Ip_aNotifIrq[IpConfig->ChannelInstanceId][ChannelIdx]     |= TypeOfEdgeInterrupt;
                        Ftm_Pwm_Ip_aNotifIrq[IpConfig->ChannelInstanceId][ChannelPairIdx] |= FTM_PWM_IP_CHF_IRQ_EDGE;
                    ...
                    case PWM_FALLING_EDGE:
                    {
                        ...
                        {
                            Ftm_Pwm_Ip_aNotifIrq[IpConfig->ChannelInstanceId][ChannelPairIdx] |= TypeOfEdgeInterrupt;
                        }
                        else
                        {
                ...
                /* Call Ftm IP initialize function */
                Ftm_Pwm_Ip_EnableNotification(IpConfig->ChannelInstanceId, NotifType);
            }
            break;
#endif
#if (PWM_FLEXIO_USED == STD_ON)
...
* @param[in]    UpdateType       Update duty cycle immediately or wait for a trigger event
...
*
*/
void Pwm_Ipw_FastUpdateDutyCycle(uint8 ModuleId,
...
{
    /* Call FTM IP function */
    Ftm_Pwm_Ip_FastUpdatePwmDuty(ModuleId, NumberOfChannels, Channels, DutyCycle, UpdateType);
}
...
/**
* @brief        This function forces channels output to their inactive state.
                         ...
                         const Pwm_IpwInstanceConfigType * const IpConfig)
{
    if (ModuleId == IpConfig->InstanceId)
    {
         /* Call FTM IP function */
#if (PWM_FTM_USED == STD_ON)
        Ftm_Pwm_Ip_MaskOutputChannels(ModuleId, ChannelsMask, UpdateType);
#endif
    }
    else
    {
                           ...
                           const Pwm_IpwInstanceConfigType * const IpConfig)
{
    ...
    if (ModuleId == IpConfig->InstanceId)
    {
        /* Call FTM IP function */
#if (PWM_FTM_USED == STD_ON)
        Ftm_Pwm_Ip_UnMaskOutputChannels(ModuleId, ChannelsMask, UpdateType);
#endif
    }
    else
    {
...
*
*/
void Pwm_Ipw_SetDutyCycle_NoUpdate(uint16                           DutyCycle,
                                   const Pwm_IpwChannelConfigType * const IpConfig)
{
