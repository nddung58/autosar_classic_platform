...
#define PDB_ADC_IP_SW_MAJOR_VERSION_TYPES               3
...
/* Check if Pdb_Adc_Ip_Types.h file and Std_Types.h file are of the same Autosar version */
    ...
    #error "AutoSar Version Numbers of Pdb_Adc_Ip_Types.h and Std_Types.h are different"
#endif
 ...
 * @brief Defines the sequence error notification header
 *
 * This header is used for interrupt delay notification callbacks
 *
 * Implements : Pdb_Adc_Ip_IntDelayNotificationType_Class
 */
typedef void Pdb_Adc_Ip_IntDelayNotificationType(const uint8 Instance);
...
#if (PDB_ADC_IP_ENABLE_INTERRUPT_DELAY == STD_ON)
    uint16 InterruptDelayPeriod;                                /*!< Interrupt Delay Period. */
    Pdb_Adc_Ip_IntDelayNotificationType * IntDelayNotification; /*!< Interrupt Delay notification. */
#endif /* (PDB_ADC_IP_ENABLE_INTERRUPT_DELAY == STD_ON) */
