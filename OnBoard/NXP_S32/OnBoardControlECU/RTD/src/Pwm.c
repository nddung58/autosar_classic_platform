...
static inline Std_ReturnType Pwm_ValidateForceOutputToZero(uint32           PartitionId,
                                                           Pwm_ChannelType  ChannelNumber
                                                          );
#endif
...
static inline Std_ReturnType Pwm_ValidateBitMask(uint32 PartitionId,
                                                 uint32 TriggerMask,
                                                 uint8  ServiceId
                                                );
#endif
...
static inline Std_ReturnType Pwm_ValidateSetOutPutToIdle(uint32            PartitionId,
                                                         Pwm_ChannelType   ChannelNumber
                                                        );
                                                          ...
                                                          )
{
    ...
    RetVal = Pwm_Ipw_ValidateForceOutputToZero(&((*Pwm_aState[PartitionId].PwmChannelConfigs)[ChannelNumber].IpwChannelCfg));
                                                ...
                                                )
{
    ...
    RetVal = Pwm_Ipw_ValidateTriggerMask(TriggerMask, &((*Pwm_aState[PartitionId].PwmChannelConfigs)[PartitionId].IpwChannelCfg));
                                                        ...
                                                        )
{
    ...
    RetVal = Pwm_Ipw_ValidateSetOutPutToIdle(&((*Pwm_aState[PartitionId].PwmChannelConfigs)[ChannelNumber].IpwChannelCfg));
...
#endif
            /* All validations passed. Here starts the actual functional code of the function */
            Pwm_Ipw_SetDutyCycle(DutyCycle, &((*Pwm_aState[PartitionId].PwmChannelConfigs)[PartitionChannelIdx].IpwChannelCfg));
...
void Pwm_SetOutputToIdle(Pwm_ChannelType ChannelNumber)
{
    ...
    if ((Std_ReturnType)E_OK == Pwm_ValidateChannelConfigCall(PartitionId, ChannelNumber, PWM_SETOUTPUTTOIDLE_ID))
    {
        ...
        PartitionChannelIdx = Pwm_GetPartitionChannelIdx(PartitionId, ChannelNumber);
#if (PWM_PARAM_CHECK == STD_ON)
        if((Std_ReturnType)E_OK == Pwm_ValidateSetOutPutToIdle(PartitionId, PartitionChannelIdx))
        {
#endif /* PWM_PARAM_CHECK */
            /* All validations passed. Here starts the actual functional code of the function */
...
*
* @param[in]    ChannelNumber   Pwm channel id
* @param[in]    bForce          Boolean value to state if the output of the channel will be forced
...
#if (PWM_VALIDATE_CHANNEL_CONFIG_CALL == STD_ON)
    if ((Std_ReturnType)E_OK == Pwm_ValidateChannelConfigCall(PartitionId, ChannelNumber, PWM_FORCEOUTPUTTOZERO_ID))
    {
#endif
        PartitionChannelIdx = Pwm_GetPartitionChannelIdx(PartitionId, ChannelNumber);

#if (PWM_PARAM_CHECK == STD_ON)
        if ((Std_ReturnType)E_OK == Pwm_ValidateForceOutputToZero(PartitionId, PartitionChannelIdx))
        {
#endif
            /* All validations passed. Here starts the actual functional code of the function */
            Pwm_Ipw_ForceOutputToZero(bForce, &((*Pwm_aState[PartitionId].PwmChannelConfigs)[PartitionChannelIdx].IpwChannelCfg));
...
#endif
            /* All validations passed. Here starts the actual functional code of the function */
            Pwm_Ipw_SetDutyCycle_NoUpdate(DutyCycle, &((*Pwm_aState[PartitionId].PwmChannelConfigs)[PartitionChannelIdx].IpwChannelCfg));
                      ...
                      )
{
...
#if (PWM_VALIDATE_GLOBAL_CONFIG_CALL == STD_ON)
    CallIsValid = Pwm_ValidateGlobalConfigCall(PartitionId, PWM_ENABLETRIGGER_ID);
    ...
    if ((Std_ReturnType)E_OK == CallIsValid)
    {
        ...
        {
            if ((Std_ReturnType)E_OK == Pwm_ValidateBitMask(PartitionId, (uint32)TriggerMask, PWM_ENABLETRIGGER_ID))
            {
#endif
                for (Index = 0 ; Index < Pwm_aState[PartitionId].PwmConfig->NumInstances; Index++)
                {
                    Pwm_Ipw_EnableTrigger(TriggerHostId, TriggerMask, &((*Pwm_aState[PartitionId].PwmConfig->PwmInstancesConfig)[Index]));
                }
    ...
    Pwm_EndValidateGlobalConfigCall(PartitionId, CallIsValid, PWM_ENABLETRIGGER_ID);
#endif
                       ...
                       )
{
...
#if (PWM_VALIDATE_GLOBAL_CONFIG_CALL == STD_ON)
    CallIsValid = Pwm_ValidateGlobalConfigCall(PartitionId, PWM_DISABLETRIGGER_ID);
    ...
    if ((Std_ReturnType)E_OK == CallIsValid)
    {
        ...
        {
            if ((Std_ReturnType)E_OK == Pwm_ValidateBitMask(PartitionId, (uint32)TriggerMask, PWM_DISABLETRIGGER_ID))
            {
#endif
                for (Index = 0 ; Index < Pwm_aState[PartitionId].PwmConfig->NumInstances; Index++)
                {
                    Pwm_Ipw_DisableTrigger(TriggerHostId, TriggerMask, &((*Pwm_aState[PartitionId].PwmConfig->PwmInstancesConfig)[Index]));
                }
    ...
    Pwm_EndValidateGlobalConfigCall(PartitionId, CallIsValid, PWM_DISABLETRIGGER_ID);
#endif
...
void Pwm_ResetCounterEnable(uint8   ModuleId)
{
...
#if (PWM_VALIDATE_GLOBAL_CONFIG_CALL == STD_ON)
    CallIsValid = Pwm_ValidateGlobalConfigCall(PartitionId, PWM_RESETCOUNTERENABLE_ID);
    ...
    if ((Std_ReturnType)E_OK == CallIsValid)
    {
        ...
        if ((Std_ReturnType)E_OK == Pwm_ValidateModuleId((uint8)ModuleId, PWM_RESETCOUNTERENABLE_ID))
        {
            ...
            for (Index = 0 ; Index < Pwm_aState[PartitionId].PwmConfig->NumInstances; Index++)
            {
                Pwm_Ipw_ResetCounterEnable(ModuleId,  &((*Pwm_aState[PartitionId].PwmConfig->PwmInstancesConfig)[Index]));
            }
    ...
    Pwm_EndValidateGlobalConfigCall(PartitionId, CallIsValid, PWM_RESETCOUNTERENABLE_ID);
#endif
...
void Pwm_ResetCounterDisable(uint8  ModuleId)
{
...
#if (PWM_VALIDATE_GLOBAL_CONFIG_CALL == STD_ON)
    CallIsValid = Pwm_ValidateGlobalConfigCall(PartitionId, PWM_RESETCOUNTERDISABLE_ID);
    ...
    if ((Std_ReturnType)E_OK == CallIsValid)
    {
        ...
        if ((Std_ReturnType)E_OK == Pwm_ValidateModuleId((uint8)ModuleId, PWM_RESETCOUNTERDISABLE_ID))
        {
            ...
            for (Index = 0 ; Index < Pwm_aState[PartitionId].PwmConfig->NumInstances; Index++)
            {
                Pwm_Ipw_ResetCounterDisable(ModuleId,  &((*Pwm_aState[PartitionId].PwmConfig->PwmInstancesConfig)[Index]));
            }
#if (PWM_PARAM_CHECK == STD_ON)
        }
#endif
    ...
    Pwm_EndValidateGlobalConfigCall(PartitionId, CallIsValid, PWM_RESETCOUNTERDISABLE_ID);
#endif
...
/**
* @brief        This function will update the duty cycle of PWM output for multiple channels with minimum overhead
* @details
*
* @param[in]    ModuleId         The FTM peripheral instance number.
...
* @param[in]    DutyCycle        The list of duty cycles for selected channels.
* @param[in]    SyncUpdate       Update duty cycle immediately or wait for a trigger event
*                                    TRUE    Update immediately
*                                    FALSE   Wait for a trigger event
*
* @return       void
*
* @implements   Pwm_FastUpdateDutyCycle_Activity
*
*/
void Pwm_FastUpdateDutyCycle(uint8 ModuleId,
                             ...
                             boolean SyncUpdate)
{
...
#if (PWM_VALIDATE_GLOBAL_CONFIG_CALL == STD_ON)
    CallIsValid = Pwm_ValidateGlobalConfigCall(PartitionId, PWM_FASTUPDATEDUTYCYCLE_ID);
...
#if (PWM_PARAM_CHECK == STD_ON)
        if ((Std_ReturnType)E_OK == Pwm_ValidateModuleId((uint8)ModuleId, PWM_FASTUPDATEDUTYCYCLE_ID))
        {
#endif
            for (Index = 0 ; Index < Pwm_aState[PartitionId].PwmConfig->NumInstances; Index++)
            {
                if (ModuleId == (*Pwm_aState[PartitionId].PwmConfig->PwmInstancesConfig)[Index].InstanceId)
                {
                    Pwm_Ipw_FastUpdateDutyCycle(ModuleId,
                                                NumberOfChannels,
                                                Channels,
                                                DutyCycle,
                                                SyncUpdate);
    ...
    Pwm_EndValidateGlobalConfigCall(PartitionId, CallIsValid, PWM_FASTUPDATEDUTYCYCLE_ID);
#endif
}
...
/**
* @brief        This function forces channels output to their inactive state (as immediate or synchronized base on API parameter UpdateType).
* @details      Corresponding bits with channels will be masked:
...
*                             TRUE     Update immediately
*                             FALSE    Wait for a trigger event (based on the synchronization when calling Pwm_SyncUpdate)
*
* @return       void
*
...
)
{
    ...
    if ((Std_ReturnType)E_OK == CallIsValid)
    {
        ...
        if ((Std_ReturnType)E_OK == Pwm_ValidateModuleId(ModuleId, PWM_MASKOUTPUT_ID))
        {
            ...
            for (Index = 0 ; Index < Pwm_aState[PartitionId].PwmConfig->NumInstances; Index++)
            {
                Pwm_Ipw_MaskOutputs(ModuleId, (uint8) ChannelsMask, UpdateType, &((*Pwm_aState[PartitionId].PwmConfig->PwmInstancesConfig)[Index]));
            }
...
#if (PWM_ENABLE_MASKING_OPERATIONS == STD_ON)
/**
* @brief        This function puts channels output to normal operation state (as immediate or synchronized base on API parameter UpdateType).
* @details      Corresponding bits with channels will be un-masked:
*               Bit 0   Channel 0 Output Mask
*               Bit 1   Channel 1 Output Mask
*               Bit 2   Channel 2 Output Mask
*               Bit 3   Channel 3 Output Mask
...
)
{
    ...
    if ((Std_ReturnType)E_OK == CallIsValid)
    {
        ...
        if ((Std_ReturnType)E_OK == Pwm_ValidateModuleId((uint8)ModuleId, PWM_UNMASKOUTPUT_ID))
        {
            ...
            for (Index = 0 ; Index < Pwm_aState[PartitionId].PwmConfig->NumInstances; Index++)
            {
                Pwm_Ipw_UnMaskOutputs(ModuleId, (uint8) ChannelsMask, UpdateType, &((*Pwm_aState[PartitionId].PwmConfig->PwmInstancesConfig)[Index]));
            }
...
* @return       void
*
* @implements   Pwm_EnableReloadNotification_Activity
*
*/
void Pwm_EnableReloadNotification(uint8 ModuleId)
{
    ...
    /* Get partition Id of current processor */
    PartitionId = Pwm_GetPartitionId();
    CallIsValid = Pwm_ValidateGlobalConfigCall(PartitionId, PWM_ENABLERELOADNOTIF_ID);
    ...
    Pwm_EndValidateGlobalConfigCall(PartitionId, CallIsValid, PWM_ENABLERELOADNOTIF_ID);
#endif
...
* @return       void
*
* @implements   Pwm_DisableReloadNotification_Activity
*
*/
void Pwm_DisableReloadNotification(uint8    ModuleId)
{
    ...
    /* Get partition Id of current processor */
    PartitionId = Pwm_GetPartitionId();
    CallIsValid = Pwm_ValidateGlobalConfigCall(PartitionId, PWM_DISABLERELOADNOTIF_ID);
    ...
    Pwm_EndValidateGlobalConfigCall(PartitionId, CallIsValid, PWM_DISABLERELOADNOTIF_ID);
#endif
