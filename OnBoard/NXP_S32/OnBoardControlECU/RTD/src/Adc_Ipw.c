                                                       ...
                                                       uint8 UserId)
{
    ...
    const uint8 NumsBitShift = Adc_Ipw_CalculateNumsBitShift(Adc_apxCfgPtr[UserId]->AdcIpwConfigPtr->AdcConfigPtrArr[LogicalHwUnitId]->Resolution);
    ...
    const uint16 DmaBufferMask = Adc_Ipw_GetAdcDataMask(Adc_apxCfgPtr[UserId]->AdcIpwConfigPtr->AdcConfigPtrArr[LogicalHwUnitId]->Resolution);
#endif /* ADC_DMA_SUPPORTED */
    ...
    {
#ifdef ADC_DMA_SUPPORTED
        if ((ADC_DMA == Adc_apxCfgPtr[UserId]->AdcIpwConfigPtr->Mapping.AdcDmaInterruptSoftware[LogicalHwUnitId]) && (FALSE == GroupPtr->AdcWithoutDma))
        {
...
#endif /* ADC_SETCHANNEL_API == STD_ON */
            (*Flag) =  Adc_Ipw_CheckConversionValuesInRange(AdcResult, LogicalHwUnitId, ChIndex, UserId);
            if (FALSE == (*Flag))
            {
                                                    ...
                                                    uint8 UserId)
{
    ...
    {
        ReadGroupRet = Adc_Ipw_ReadGroupConvData(GroupPtr, DataPtr, Flag, UserId);
                                           ...
                                           uint8 UserId)
{
    ...
    uint16 DmaBufferMask = Adc_Ipw_GetAdcDataMask(Adc_apxCfgPtr[UserId]->AdcIpwConfigPtr->AdcConfigPtrArr[LogicalHwUnitId]->Resolution);
#endif /* ADC_DMA_SUPPORTED */
#if (ADC_SETCHANNEL_API == STD_ON)
    const Adc_ChannelIndexType ChannelCount = Adc_axRuntimeGroupChannel[Group].ChannelCount;
#else
                                                    ...
                                                    uint8 NumChannel)
{
    ...
    if (FALSE == Pdb_Adc_Ip_GetInterruptDelayState(PhysicalHwUnitId))
    {
        Pdb_Adc_Ip_InterruptDelayEnable(PhysicalHwUnitId);
    }
...
#if (PDB_ADC_IP_ENABLE_INTERRUPT_DELAY == STD_ON)
    if (TRUE == Pdb_Adc_Ip_GetInterruptDelayState(PhysicalHwUnitId))
    {
        Pdb_Adc_Ip_InterruptDelayDisable(PhysicalHwUnitId);
    }
#endif /* (PDB_ADC_IP_ENABLE_INTERRUPT_DELAY == STD_ON) */
    /* Check if ADC is in continuous mode */
    if (((uint8)STD_ON == Adc_apxCfgPtr[UserId]->GroupsPtr[GroupIndex].AdcWithoutInterrupt) && (ADC_CONV_MODE_CONTINUOUS == Adc_apxCfgPtr[UserId]->GroupsPtr[GroupIndex].Mode))
    {
        /* Set to Oneshot mode */
                               ...
                               uint8 UserId)
{
    ...
    const uint32 AdcDmaLogicChId = Adc_apxCfgPtr[UserId]->AdcIpwConfigPtr->Mapping.AdcDmaChannelLogicId[Unit];
    ...
    const uint32 AdcCountingDmaLogicChId = Adc_apxCfgPtr[UserId]->AdcIpwConfigPtr->Mapping.AdcCountingDmaChanLogicId[Unit];
                                 ...
                                 uint8 UserId)
{
    ...
    for (ChIndex = 0U; ChIndex < (NumChannel - 1U); ChIndex++)
    {
...
#endif /* ADC_SETCHANNEL_API */
        {
            ...
            /* Physical channel ID */
            ChannelConfig.Channel = Adc_apxCfgPtr[UserId]->AdcIpwConfigPtr->AdcConfigPtrArr[Unit]->ChannelConfigs[ChannelsDefinition[(CurrentChannel + ChIndex)]].Channel;
                                   ...
                                   uint8 UserId)
{
    ...
    if ((ADC_DMA == Adc_apxCfgPtr[UserId]->AdcIpwConfigPtr->Mapping.AdcDmaInterruptSoftware[Unit]))
    {
        ...
        if ((uint8)STD_OFF == GroupPtr->AdcExtDMAChanEnable)
        {
            ...
            Adc_Ipw_StartDmaOperation(Unit, Group, NumChannel, UserId);
        }
...
#endif /* ADC_DMA_SUPPORTED */
    {
        ...
        Adc_Ipw_ConfigureConversion(Unit, Group, GroupPtr->HwTriggerSource, NumChannel, UserId);
    }
#if (ADC_SETCHANNEL_API == STD_ON)
                                   ...
                                   uint8 UserId)
{
    ...
    Adc_GroupType GroupIndex = Adc_apxCfgPtr[UserId]->GroupIdToIndexMapPtr[Group]; /* Group index in each partition */
    const Adc_GroupConfigurationType * GroupPtr = &(Adc_apxCfgPtr[UserId]->GroupsPtr[GroupIndex]); /* Pointer to AdcGroup */
#endif /* (defined(ADC_DMA_SUPPORTED) ||... */
    const Adc_HwUnitType PhysicalHwUnitId = Adc_apxCfgPtr[UserId]->AdcIpwConfigPtr->AdcPhysicalIdArr[Unit];
    uint8 NumChannel;
       ...
       )
    {
...
#endif /* ADC_DMA_SUPPORTED */
        {
            ...
            Adc_Ipw_ConfigureConversion(Unit, Group, PDB_ADC_IP_SOFTWARE_TRIGGER, NumChannel, UserId);
        }
#if (ADC_SETCHANNEL_API == STD_ON)
        if (TRUE == Adc_axRuntimeGroupChannel[Group].RuntimeUpdated)
        {
                                             ...
                                             uint8 UserId)
{
...
#ifdef ADC_DMA_SUPPORTED
    if (((uint8)ADC_DMA == Adc_apxCfgPtr[UserId]->AdcIpwConfigPtr->Mapping.AdcDmaInterruptSoftware[Unit]) && (FALSE == Adc_apxCfgPtr[UserId]->GroupsPtr[GroupIndex].AdcWithoutDma))
    {
        /* Disable DMA for Adc */
    ...
    if ((uint8)STD_OFF == Adc_apxCfgPtr[UserId]->GroupsPtr[GroupIndex].AdcWithoutInterrupt)
    {
        Status = Adc_Ipw_StopConversionCheckTimeout(Unit, Group, UserId);
    }
#ifdef ADC_CONTINUOUS_NO_INTERRUPT_SUPPORTED
    else
    {
                                             ...
                                             uint8 UserId)
{
    ...
    Adc_Ipw_StartHwTrigConversion(Unit, Group, UserId);
                                 ...
                                 uint8 UserId)
{
    ...
    if ((uint8)STD_ON == GroupPtr->AdcWithoutInterrupt)
    {
        ...
        {
            ReadGroupRet = Adc_Ipw_ReadGroupNoInt(GroupPtr, DataPtr, Flag, UserId);
        }
    }
    else  /* If the group is configured with interrupts */
    {
        ...
        if ((Std_ReturnType)E_OK == ReadGroupRet)
        {
            Adc_Ipw_WriteDataBuffer(Group, DataPtr, UserId);
        }
    }
...
boolean Adc_Ipw_CheckConversionValuesInRange(Adc_ValueGroupType Value,
                                             ...
                                             uint8 UserId)
{
    const Adc_ChannelLimitCheckingType * const ChannelLimitCheckingCfg = &(Adc_apxCfgPtr[UserId]->AdcIpwConfigPtr->ChannelLimitCheckingConfigs[LogicalHwUnitId][ChannelIndex]);
                                    ...
                                    uint8 UserId)
{
    ...
    for (LogicalHwUnitId = 0U; LogicalHwUnitId < ADC_IPW_MAX_HW_UNITS_CFG; LogicalHwUnitId++)
    {
        if ((uint8)STD_ON == Adc_apxCfgPtr[UserId]->AdcIpwConfigPtr->Mapping.AdcHwUnitArr[LogicalHwUnitId]) /* HW unit enabled on current partition */
        {
            PhysicalHwUnitId = Adc_apxCfgPtr[UserId]->AdcIpwConfigPtr->AdcPhysicalIdArr[LogicalHwUnitId];
            AdcIpConfigPtr = Adc_apxCfgPtr[UserId]->AdcIpwConfigPtr->AdcConfigPtrArr[LogicalHwUnitId];
            ...
            {
                Adc_Ip_SetClockMode(PhysicalHwUnitId, Adc_apxCfgPtr[UserId]->AdcIpwConfigPtr->AdcAltClockConfig[LogicalHwUnitId]);
            }
        }
    }
                       ...
                       uint8 UserId)
{
    ...
    if ((IpStatus != ADC_IP_STATUS_SUCCESS) && (IpStatus != ADC_IP_STATUS_CAL_ONGOING))
#else
                                            ...
                                            const Adc_UserOffsetType Offset)
{
    ...
    Adc_Ip_SetUserGainAndOffset(PhysicalHwUnitId, (uint16)Gain, (uint8)Offset);
