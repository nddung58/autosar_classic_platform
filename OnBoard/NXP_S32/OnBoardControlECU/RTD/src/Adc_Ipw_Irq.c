                                            ...
                                            uint8 UserId)
{
       ...
       )
    {
        ...
        Adc_apxCfgPtr[UserId]->GroupsPtr[GroupIndex].Notification();
    }
}
#endif /* ADC_GRP_NOTIF_CAPABILITY == STD_ON */
                                                            ...
                                                            uint8 UserId)
{
    ...
    const uint8 NumsBitShift = Adc_Ipw_CalculateNumsBitShift(Adc_apxCfgPtr[UserId]->AdcIpwConfigPtr->AdcConfigPtrArr[Unit]->Resolution);
#if (ADC_ENABLE_LIMIT_CHECK == STD_ON)
...
#else
    Index = Adc_apxCfgPtr[UserId]->GroupsPtr[GroupIndex].AssignedChannelCount;
#endif /* (ADC_SETCHANNEL_API == STD_ON) */
...
#else
                    HwChannel = Adc_apxCfgPtr[UserId]->GroupsPtr[GroupIndex].AssignmentPtr[Index];
#endif /* (ADC_SETCHANNEL_API == STD_ON) */
                    /* SWS_Adc_00446, SWS_Adc_00447 */
                    ResultInRange = Adc_Ipw_CheckConversionValuesInRange(ConvResult, Unit, HwChannel, UserId);
                                                     ...
                                                     uint8 UserId)
{
    ...
    else
    {
        ...
        Adc_Ipw_ConfigureConversion(Unit, Group, PDB_ADC_IP_SOFTWARE_TRIGGER, 1U, UserId);
    }
}
#endif /* (ADC_ENABLE_LIMIT_CHECK == STD_ON) */
                                             ...
                                             uint8 UserId)
{
    ...
    {
        Adc_Ipw_ConfigureRuntimeChannels(Unit, GroupPtr, NumChannel, UserId);
    }
    else
#endif  /* (ADC_SETCHANNEL_API == STD_ON) */
    {
                                                  ...
                                                  )
{
    ...
    const Adc_GroupType GroupIndex = Adc_apxCfgPtr[UserId]->GroupIdToIndexMapPtr[Group];
    const Adc_Ipw_Config * const LocIpwConfigPtr = Adc_apxCfgPtr[UserId]->AdcIpwConfigPtr;
...
#ifdef ADC_DMA_SUPPORTED
#if (STD_ON == ADC_OPTIMIZE_DMA_STREAMING_GROUPS)
    if (TRUE == Adc_apxCfgPtr[UserId]->GroupsPtr[GroupIndex].AdcOptimizeDmaStream)
    {
        if (1U < Adc_apxCfgPtr[UserId]->GroupsPtr[GroupIndex].AssignedChannelCount)
        {
                                                    ...
                                                    uint8 UserId)
{
...
#if (STD_ON == ADC_OPTIMIZE_DMA_STREAMING_GROUPS)
    CurrentSampleCount = Adc_Ipw_GetCurrentSampleCount(Group, Unit, GroupSamples, UserId);
#else
    ...
    if (CurrentSampleCount >= GroupSamples)
    {
        ...
        if ((ADC_STREAM_BUFFER_LINEAR == Adc_apxCfgPtr[UserId]->GroupsPtr[GroupIndex].BufferMode) && \
...
#endif /*ADC_DMA_SUPPORTED*/
                Adc_Ipw_RestartConversion(Unit, Group, UserId);
            }
            else
            {
    ...
    Adc_Ipw_CallNotification(Group, UserId);
#endif /* (ADC_GRP_NOTIF_CAPABILITY == STD_ON) */
}
                                                         ...
                                                         uint8 UserId)
{
    ...
    Dma_Ip_LogicChannelScatterGatherListType LocTransferList[1U];
    /* ADC_ACCESS_MODE_STREAMING && ADC_STREAM_BUFFER_CIRCULAR */
    if ((ADC_STREAM_BUFFER_CIRCULAR == Adc_apxCfgPtr[UserId]->GroupsPtr[GroupIndex].BufferMode) && \
        (ADC_ACCESS_MODE_STREAMING == Adc_apxCfgPtr[UserId]->GroupsPtr[GroupIndex].AccessMode)
        )
    {
        if (ADC_DMA == Adc_apxCfgPtr[UserId]->AdcIpwConfigPtr->Mapping.AdcDmaInterruptSoftware[Unit])
                                                    ...
                                                    uint8 UserId)
{
    ...
    if (CurrentSampleCount >= GroupSamples)
    {
        ...
        else
        {
            Adc_axGroupStatus[Group].ResultIndex = 0U;
#ifdef ADC_DMA_SUPPORTED
            Adc_Ipw_CheckAndUpdateDmaHwGroupState(Unit, Group, AdcDmaLogicChId, UserId);
#endif /* ADC_DMA_SUPPORTED */
        }
    }
                                         ...
                                         uint8 UserId)
{
    ...
    if (TRUE == Adc_axRuntimeGroupChannel[Group].RuntimeUpdated)
    {
        Adc_Ipw_ConfigureRuntimeChannels(Unit, GroupPtr, (uint8)ADC_IPW_CALCULATE_CHAN_NUMBER(ChannelCount), UserId);
    }
    else
#endif /* (ADC_SETCHANNEL_API == STD_ON) */
    {
        /* Limit checking */
        if ((Std_ReturnType)E_OK == Adc_Ipw_CheckConversionChannels(Unit, Group, GroupPtr->NumSamples, UserId))
        {
#if (ADC_OPTIMIZE_ONESHOT_HW_TRIGGER == STD_OFF)
            ...
            if (Adc_axGroupStatus[Group].CurrentChannel < ChannelCount)
            {
                ...
                Adc_Ipw_ConfigureConversion(Unit, Group, PDB_ADC_IP_SOFTWARE_TRIGGER, LeftChannel, UserId);
            }
            ...
            (void)IsSoftwareConversion;
                ...
                * ADC1 will not have the same group as an interrupt on ADC0 */
                ...
                {
                    Adc_Ipw_UpdateSoftwareGroupState(Unit, Group, GroupPtr->NumSamples, UserId);
                }
#if (ADC_HW_TRIGGER_API == STD_ON)
                else
                {
                    Adc_Ipw_UpdateHardwareGroupState(Unit, Group, GroupPtr->NumSamples, UserId);
                }
#endif  /* (ADC_HW_TRIGGER_API == STD_ON) */
        ...
        else
        {
            if (TRUE == IsSoftwareConversion)
            {
                Adc_Ipw_HandleConversionCheckFail(Unit, Group, GroupPtr, UserId);
            }
        }
#endif /* (ADC_ENABLE_LIMIT_CHECK == STD_ON) */
    }
                                            ...
                                            uint8 UserId)
{
    ...
    {
#if (ADC_ENABLE_LIMIT_CHECK == STD_ON)
        if ((Std_ReturnType)E_OK == Adc_Ipw_CheckGroupConvResultDma(GroupPtr, UserId))
#endif /* (ADC_ENABLE_LIMIT_CHECK == STD_ON) */
        {
#if (ADC_OPTIMIZE_ONESHOT_HW_TRIGGER == STD_OFF)
            /* Following code will not be reduced when oneshot hardware trigger optimization is not enabled */
            ...
            if (Adc_axGroupStatus[Group].CurrentChannel < ChannelCount)
            {
                Adc_Ipw_ContinueDmaConversion(Unit, Group, ChannelCount, AdcDmaLogicChId, UserId);
            }
            /* All of channels converted */
            else
#endif /* (ADC_OPTIMIZE_ONESHOT_HW_TRIGGER == STD_OFF) */
                                                             ...
                                                             uint8 UserId)
{
       ...
       )
    {
        ...
        if (FALSE == Adc_Ipw_CheckConversionValuesInRange(ResultDataPtr[0], Unit, HwChannel, UserId))
        {
            /* SWS_Adc_00448 */
            /* Return an error */
            Adc_axGroupStatus[Group].AlreadyConverted = ADC_ALREADY_CONVERTED;
...
static inline void Adc_Ipw_EndConversionNotification(const Adc_HwUnitType PhysicalAdcUnit)
{
    ...
    const Adc_HwUnitType LogicalHwUnitId = Adc_apxCfgPtr[UserId]->AdcIpwConfigPtr->Mapping.HwLogicalId[PhysicalAdcUnit];
    ...
    if (ADC_INVALID_HW_GROUP_ID != HwGroupId)
    {
        ...
        {
            Adc_Ipw_EndDmaConversion(LogicalHwUnitId, HwGroupId, FALSE, UserId);
        }
        else
#endif /* ADC_DMA_SUPPORTED */
        {
            Adc_Ipw_EndConversion(LogicalHwUnitId, HwGroupId, FALSE, UserId);
        }
    }
    else
#endif /* (ADC_HW_TRIGGER_API == STD_ON) */
    if (Adc_axUnitStatus[LogicalHwUnitId].SwNormalQueueIndex > (Adc_QueueIndexType)0U)
    {
        ...
        if (ADC_DMA == Adc_apxCfgPtr[UserId]->AdcIpwConfigPtr->Mapping.AdcDmaInterruptSoftware[LogicalHwUnitId])
        {
            Adc_Ipw_EndDmaConversion(LogicalHwUnitId, SwGroupId, TRUE, UserId);
        }
        else
#endif /* ADC_DMA_SUPPORTED */
        {
            Adc_Ipw_EndConversion(LogicalHwUnitId, SwGroupId, TRUE, UserId);
        }
    }
#if (ADC_HW_TRIGGER_API == STD_ON)
    else
